    ...
    if column_mapping:
        ...
        for field in column_fields:
            ...
            if not mapped_col or mapped_col not in data.columns:
                ...
                checks['validation_errors'].append(f"Column '{mapped_col}' mapped to '{field}' contains only missing values")
        
        # Check source_scale separately (it's now an enum value, not a column)
        source_scale_value = column_mapping.get('source_scale')
        if not source_scale_value:
            missing_fields.append('source_scale')
        elif str(source_scale_value).lower() not in ['site', 'equipment', 'component']:
            checks['validation_errors'].append(
                f"Source scale must be one of: site, equipment, or component. Got: {source_scale_value}"
            )
            checks['mapping_valid'] = False
        
        if missing_fields:
            checks['validation_errors'].append(f"Missing mandatory field mappings: {', '.join(missing_fields)}")
            checks['mapping_valid'] = False
        else:
            ...
            if rate_col and rate_col in data.columns:
                if not pd.api.types.is_numeric_dtype(data[rate_col]):
                    checks['validation_errors'].append(f"Rate column '{rate_col}' must be numeric")
                    checks['mapping_valid'] = False
            
            id_col = column_mapping.get('id')
            if id_col and id_col in data.columns:
                if data[id_col].duplicated().any():
                    checks['validation_errors'].append(f"ID column '{id_col}' contains duplicate values")
                    checks['mapping_valid'] = False
    else:
        # Check if data already has required columns (auto-detection)
        has_rate = 'rate' in data.columns or any('rate' in str(col).lower() for col in data.columns)
        has_id = 'id' in data.columns or any('id' in str(col).lower() for col in data.columns)
        has_source = 'source' in data.columns or any('source' in str(col).lower() for col in data.columns)
...
def switch_section(data_inv_clicks, uncertainty_clicks, sim_clicks, results_clicks, workflow_status):
    ...
    step1_completed = workflow_status.get('step1_completed', False) if workflow_status else False
    step2_completed = workflow_status.get('step2_completed', False) if workflow_status else False
    
    # Determine button styles based on workflow status
    uncertainty_button_style = get_disabled_nav_style() if not step1_completed else get_inactive_nav_style()
    simulation_button_style = get_disabled_nav_style() if not step2_completed else get_inactive_nav_style()
    
    ctx = callback_context
    if not ctx.triggered:
        # Default to Loading Emissions Data
        return (get_data_investigation_content(), 
    ...
    if button_id == 'section-data-investigation-button':
        ...
        return (get_event_uncertainty_calculator_content(), 
                get_inactive_nav_style(), 
                get_active_nav_style(), 
                simulation_button_style, 
                get_inactive_nav_style())
    elif button_id == 'section-simulation-button':
        return (get_simulation_selection_content(), 
                get_inactive_nav_style(), 
                get_inactive_nav_style(), 
                get_active_nav_style(), 
                get_inactive_nav_style())
    elif button_id == 'section-results-button':
        return (get_results_content(), 
                get_inactive_nav_style(), 
                get_inactive_nav_style(), 
                get_inactive_nav_style(), 
                get_active_nav_style())
...
def update_navigation_button_styles(workflow_status):
    ...
    uncertainty_style = get_disabled_nav_style() if not step1_completed else get_inactive_nav_style()
    simulation_style = get_disabled_nav_style() if not step2_completed else get_inactive_nav_style()
...
def update_data(contents, filename):
    ...
    status = html.Div([
        html.P(f"✓ Successfully loaded: {filename} ({len(df)} rows, {len(df.columns)} columns)",
              style={'color': '#27ae60', 'fontWeight': 'bold'})
    ])
    
    button = html.Button('Map Columns', id='open-mapping-modal-button', n_clicks=0,
...
def restore_data_display(section_content, stored_data, column_mapping, stored_events):
    ...
    if already_mapped:
        ...
        status = html.Div([
            html.P(f"✓ Data loaded ({len(df)} rows, {len(df.columns)} columns)",
                  style={'color': '#27ae60', 'fontWeight': 'bold'}),
            html.P("✓ Columns already mapped and events converted",
...
@app.callback(
    [Output('mapping-modal', 'style'),
     Output('mapping-modal-backdrop', 'style')],
...
def toggle_mapping_modal(open_clicks, close_clicks, cancel_clicks, backdrop_clicks, modal_style, backdrop_style):
    ...
    if button_id in ['close-mapping-modal', 'cancel-mapping-button', 'mapping-modal-backdrop']:
...
def update_mapping_dropdowns(stored_data):
    ...
    Order: ID, Rate, Time, Source, Cause, Start Time, End Time"""
    if stored_data is None:
...
def process_data_workflow(n_clicks, data, rate_col, id_col, source_scale_col, source_col, cause_col, start_time_col, end_time_col, time_col, modal_style, backdrop_style):
    """
    ...
    # Note: source_scale is now a direct enum value, not a column name
    # Ensure source_scale_col is properly handled (it should be 'site', 'equipment', or 'component')
    # Use default 'site' if not provided (since dropdown has default value)
    source_scale_value = source_scale_col if source_scale_col else 'site'
    ...
    mapping = {
        ...
        'source_scale': source_scale_value,  # This is now the enum value ('site', 'equipment', or 'component')
    ...
    # Check that source_scale enum is selected
    # Handle both string and None/empty cases
    source_scale_val = mapping.get('source_scale')
    if not source_scale_val:
        missing.append('source_scale')
    elif isinstance(source_scale_val, str) and source_scale_val.strip() == '':
        missing.append('source_scale')
    elif str(source_scale_val).lower() not in ['site', 'equipment', 'component']:
        missing.append('source_scale')
    
    if missing:
        mapping_status = html.Div([
            html.P(f"⚠ Missing mandatory field mappings: {', '.join(missing)}", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold'})
        ])
        # Keep modal open on error
        return None, mapping_status, html.Div(), None, html.Div(), html.Div(), html.Div(), modal_style, backdrop_style, mapping_status
    ...
    try:
        ...
        if validation_errors:
            validation_status = html.Div([
                html.P("⚠ Data validation issues:", 
                      style={'color': '#e67e22', 'fontWeight': 'bold', 'marginTop': '10px'}),
                html.Ul([html.Li(error, style={'color': '#e67e22'}) for error in validation_errors],
                       style={'marginLeft': '20px'})
            ])
            # Keep modal open on error
            return mapping, mapping_status, validation_status, None, html.Div(), html.Div(), html.Div(), None, modal_style, backdrop_style, validation_status
        else:
            validation_status = html.Div([
                html.P("✓ Data format validated successfully", 
                      style={'color': '#27ae60', 'fontWeight': 'bold', 'marginTop': '10px'})
            ])
    except Exception as e:
        validation_status = html.Div([
            html.P(f"✗ Validation error: {str(e)}", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold', 'marginTop': '10px'})
        ])
        # Keep modal open on error
        return mapping, mapping_status, validation_status, None, html.Div(), html.Div(), html.Div(), None, modal_style, backdrop_style, validation_status
    ...
    try:
        ...
        emission_characteristics = mapping.get('emission_characteristics', 'intermittent')
        events_df = convert_to_emission_event_model(df, mapping, time_column=time_column, emission_characteristics=emission_characteristics)
        
        conversion_status = html.Div([
            html.P(f"✓ Converted {len(events_df)} events to Emission Event model", 
                  style={'color': '#27ae60', 'fontWeight': 'bold', 'marginTop': '10px'}),
            html.P("✓ Events stored in memory", 
            ...
            html.Details([
                ...
                html.Ul([
                    ...
                    html.Li("Source: sourceLocation, sourceCategory, physicalSource")
                ])
        ...
        try:
            ...
            sankey_fig_dict = sankey_fig.to_dict() if hasattr(sankey_fig, 'to_dict') else None
            sankey_chart = dcc.Graph(figure=sankey_fig, id='sankey-chart')
        except Exception as e:
            sankey_fig_dict = None
            sankey_chart = html.Div([
                html.P(f"Could not generate Sankey chart: {str(e)}", 
        ...
        return mapping, mapping_status, validation_status, events_df.to_dict('records'), events_table, conversion_status, sankey_chart, sankey_fig_dict, new_modal_style, new_backdrop_style, html.Div()
        
    except ValueError as e:
        conversion_status = html.Div([
            html.P(f"✗ Error converting data: {str(e)}", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold', 'marginTop': '10px'})
        ])
        # Keep modal open on error
        return mapping, mapping_status, validation_status, None, html.Div(), conversion_status, html.Div(), None, modal_style, backdrop_style, conversion_status
    except Exception as e:
        conversion_status = html.Div([
...
def restore_converted_events(section_content, stored_events):
    ...
    status = html.Div([
        html.P(f"✓ {len(events_df)} events loaded from memory", 
              style={'color': '#27ae60', 'fontWeight': 'bold'})
    ])
...
@app.callback(
    Output('sankey-chart-container', 'children', allow_duplicate=True),
    [Input('section-content', 'children'),
     Input('stored-sankey-chart', 'data')],
    prevent_initial_call=True
)
def restore_sankey_chart(section_content, sankey_chart_data):
    ...
    try:
        ...
        return html.Div([
            html.P(f"Could not restore Sankey chart: {str(e)}", 
                  style={'color': '#e67e22', 'fontStyle': 'italic'})
        ])
...
def plot_histograms_with_fitting(events_data, fit_clicks, dist_type):
    if events_data is None:
        empty_fig = go.Figure()
        empty_fig.add_annotation(text="Please convert data first", 
    ...
    if 'rate' in events_df.columns:
        ...
        if len(rate_data) > 0:
            ...
            fig_rate.update_layout(
                title='Rate Distribution' + (f' with Fitted {dist_type}' if show_fitted else ''),
            ...
            fig_rate = go.Figure()
            fig_rate.add_annotation(text="No rate data available", 
                                  xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
    else:
        fig_rate = go.Figure()
        fig_rate.add_annotation(text="Rate data not available", 
                              xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
    ...
    if 'duration' in events_df.columns:
        ...
        if len(duration_data) > 0:
            ...
            fig_duration.update_layout(
                title='Duration Distribution' + (f' with Fitted {dist_type}' if show_fitted else ''),
            ...
            fig_duration = go.Figure()
            fig_duration.add_annotation(text="No duration data available", 
                                       xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
    else:
        fig_duration = go.Figure()
        fig_duration.add_annotation(text="Duration data not available", 
                                   xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
    
    return fig_rate, fig_duration


@app.callback(
    Output('iterations-display', 'children'),
    [Input('monte-carlo-iterations', 'value')]
)
...
def handle_leak_data_upload(contents, upload_choice, filename):
    ...
    if upload_choice == 'yes':
        ...
        if trigger_id == 'upload-leak-data-dropdown':
            ...
            return (html.Div([
                html.P("Please upload your CSV file.", 
                      style={'color': '#7f8c8d', 'fontStyle': 'italic'})
            ]), None, None, {'marginTop': '10px', 'display': 'none'}, [], None, {'marginTop': '20px', 'display': 'none'})
        
        if contents is None:
            return html.Div(), None, None, {'marginTop': '10px', 'display': 'none'}, [], None, {'marginTop': '20px', 'display': 'none'}
        ...
        try:
            ...
            status = html.Div([
                html.P(f"✓ File '{filename}' uploaded successfully. Found {len(df)} rows and {len(df.columns)} columns.", 
                      style={'color': '#27ae60', 'fontWeight': 'bold'}),
                html.P("Please select the column containing leak rates.", 
                      style={'color': '#7f8c8d', 'fontSize': '14px', 'marginTop': '5px'})
            ])
            ...
            return (html.Div([
                html.P(f"✗ Error reading file: {str(e)}", 
                      style={'color': '#e74c3c', 'fontWeight': 'bold'})
            ]), None, None, {'marginTop': '10px', 'display': 'none'}, [], None, {'marginTop': '20px', 'display': 'none'})
        ...
        try:
            ...
            if os.path.exists(sample_file_path):
                ...
                if 'gpersec' in df.columns:
                    ...
                    status = html.Div([
                        html.P(f"✓ Loaded sample_leak_rate.csv. Found {len(leak_data)} data points.", 
                              style={'color': '#27ae60', 'fontWeight': 'bold'})
                    ])
                    ...
                    return (html.Div([
                        html.P("⚠ 'gpersec' column not found in sample_leak_rate.csv", 
                              style={'color': '#e67e22', 'fontWeight': 'bold'})
                    ]), None, None, {'marginTop': '10px', 'display': 'none'}, [], None, {'marginTop': '20px', 'display': 'none'})
            else:
                return (html.Div([
                    html.P("⚠ sample_leak_rate.csv not found in the application directory.", 
                          style={'color': '#e67e22', 'fontWeight': 'bold'})
                ]), None, None, {'marginTop': '10px', 'display': 'none'}, [], None, {'marginTop': '20px', 'display': 'none'})
                
        except Exception as e:
            return (html.Div([
                html.P(f"✗ Error loading sample file: {str(e)}", 
                      style={'color': '#e74c3c', 'fontWeight': 'bold'})
            ]), None, None, {'marginTop': '10px', 'display': 'none'}, [], None, {'marginTop': '20px', 'display': 'none'})
    ...
    try:
        ...
        if selected_column not in df.columns:
            return (None, 
                   {'marginTop': '20px', 'display': 'none'},
                   html.Div([
                       html.P(f"⚠ Selected column '{selected_column}' not found in CSV.", 
                             style={'color': '#e67e22', 'fontWeight': 'bold'})
                   ]))
        ...
        if len(leak_data) == 0:
            return (None,
                   {'marginTop': '20px', 'display': 'none'},
                   html.Div([
                       html.P(f"⚠ No valid data found in column '{selected_column}'.", 
                             style={'color': '#e67e22', 'fontWeight': 'bold'})
                   ]))
        
        status = html.Div([
            html.P(f"✓ Column '{selected_column}' selected. Found {len(leak_data)} data points.", 
                  style={'color': '#27ae60', 'fontWeight': 'bold'})
        ])
        
        return ({'data': leak_data, 'source': 'uploaded', 'column': selected_column}, 
               {'marginTop': '20px', 'display': 'block'},
               status)
        ...
        return (None,
               {'marginTop': '20px', 'display': 'none'},
               html.Div([
                   html.P(f"✗ Error processing column selection: {str(e)}", 
                         style={'color': '#e74c3c', 'fontWeight': 'bold'})
               ]))
    ...
    if upload_choice == 'upload':
        ...
        if trigger_id == 'wind-data-source-dropdown':
            ...
            return (html.Div([
                html.P("Please upload your CSV file.", 
                      style={'color': '#7f8c8d', 'fontStyle': 'italic'})
            ]), None, None, {'marginTop': '10px', 'display': 'none'}, [], None)
        
        if contents is None:
            return html.Div(), None, None, {'marginTop': '10px', 'display': 'none'}, [], None
        ...
        try:
            ...
            status = html.Div([
                html.P(f"✓ File '{filename}' uploaded successfully. Found {len(df)} rows and {len(df.columns)} columns.", 
                      style={'color': '#27ae60', 'fontWeight': 'bold'}),
                html.P("Please select the column containing wind speed (m/s).", 
                      style={'color': '#7f8c8d', 'fontSize': '14px', 'marginTop': '5px'})
            ])
...
def handle_wind_column_selection_and_default(selected_column, upload_choice, consider_wind, csv_store):
    ...
    if upload_choice == 'upload':
        ...
        try:
            ...
            status = html.Div([
                html.P(f"✓ Column '{selected_column}' selected. Found {len(wind_data)} data points.", 
                      style={'color': '#27ae60', 'fontWeight': 'bold'})
            ])
            
            return ({'data': wind_data, 'source': 'uploaded', 'column': selected_column}, status)
    ...
    # Handle default data
        ...
        try:
            ...
            if not HAS_NETCDF:
                return None, html.Div([
                    html.P("⚠ netCDF4 library is not available. Cannot load wind data.", 
                          style={'color': '#e67e22', 'fontWeight': 'bold'})
                ])
            ...
            if os.path.exists(weather_file_path):
                ...
                if len(wind_speed_list) > 0:
                    status = html.Div([
                        html.P("✓ Using default wind data (weather_permian.nc)", 
                              style={'color': '#27ae60', 'fontWeight': 'bold'})
                    ])
                    return {'data': wind_speed_list, 'source': 'default', 'file': 'weather_permian.nc'}, status
            else:
                return None, html.Div([
                    html.P("⚠ weather_permian.nc file not found.", 
                          style={'color': '#e67e22', 'fontWeight': 'bold'})
                ])
        except Exception as e:
            return None, html.Div([
                html.P(f"✗ Error loading wind data: {str(e)}", 
                      style={'color': '#e74c3c', 'fontWeight': 'bold'})
            ])
...
def toggle_approach_specific_fields(estimation_approach):
    ...
    if estimation_approach == 'below_MDL':
        ...
        return (
            {'marginBottom': '20px', 'display': 'block', 'visibility': 'hidden', 'height': '0', 'overflow': 'hidden', 'position': 'absolute'},
...
def toggle_mdl_and_technology(measurement_technology):
    ...
    if measurement_technology == 'define_technology':
        # Show MDL input
        return {'marginBottom': '20px', 'display': 'block'}
    else:
        # Hide MDL input
...
def run_simulation_callback(n_clicks, events_data, processing_resolution, estimation_approach, 
                            monte_carlo_iterations, measurement_technology, minimum_detection_limit,
                            start_date, end_date, consider_wind, producing_hours_data, emission_char_data, wind_data_store, leak_data_store, results_button_clicks):
    if n_clicks == 0:
        raise PreventUpdate
    
    if events_data is None:
        return None, html.Div([
            html.P("Please convert data to emission event model first (go to Loading Emissions Data section)", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold'})
    ...
    # Show notification for high MC iterations
    HIGH_ITERATIONS_THRESHOLD = 20
    if monte_carlo_iterations >= HIGH_ITERATIONS_THRESHOLD:
        # Estimate time (rough estimate: ~1-2 seconds per iteration for small datasets, more for large)
        estimated_minutes = max(1, monte_carlo_iterations // 30)  # Rough estimate
        status_message = html.Div([
            html.Div([
                html.H4("⏳ Simulation Running", style={
                    'color': '#3498db',
                    'marginBottom': '10px',
                    'fontSize': '18px',
                    'fontWeight': 'bold'
                ...
                html.P([
                    f"Running simulation with {monte_carlo_iterations} Monte Carlo iterations. ",
                    "This may take several minutes. ",
                    f"Estimated time: ~{estimated_minutes} minute{'s' if estimated_minutes > 1 else ''}."
                ], style={
                    'color': '#2c3e50',
                    'marginBottom': '10px',
                    'fontSize': '14px',
                ...
                html.P([
                    "💡 Tip: The simulation is using parallel processing to speed up computation. ",
                    "You will be automatically redirected to the Results page when complete."
                ], style={
                    'color': '#7f8c8d',
                    'fontSize': '13px',
                    'fontStyle': 'italic',
        ...
        status_message = html.Div([
            html.P("🔄 Running simulation...", style={
                'color': '#3498db',
                'fontSize': '14px',
                'fontStyle': 'italic',
                'marginBottom': '10px'
    ...
    mdl_value = None
    tech_value = measurement_technology
    
    if measurement_technology == 'define_technology' and minimum_detection_limit is not None:
    ...
    if start_datetime is None or end_datetime is None:
        return None, html.Div([
            html.P("Please select both start date and end date for the simulation", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold'})
        ]), results_button_clicks or 0
    ...
    if date_diff < 0:
        return None, html.Div([
            html.P("End date must be after start date", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold'})
        ]), results_button_clicks or 0
    if date_diff > 365:
        return None, html.Div([
            html.P("Simulation period must be within a year (365 days)", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold'})
        ]), results_button_clicks or 0
    ...
    try:
        if estimation_approach == 'below_MDL':
            ...
            if 'duration' not in events_for_simulation.columns:
                return None, html.Div([
                    html.P("Missing 'duration' column in emission events.", 
                          style={'color': '#e74c3c', 'fontWeight': 'bold'})
                ]), results_button_clicks or 0
            
            # Handle source column (may be named 'sourceLocation' in converted events)
            if 'sourceLocation' in events_for_simulation.columns:
                events_for_simulation['source'] = events_for_simulation['sourceLocation']
            elif 'source' not in events_for_simulation.columns:
                return None, html.Div([
                    html.P("Missing 'source' or 'sourceLocation' column in emission events.", 
                          style={'color': '#e74c3c', 'fontWeight': 'bold'})
                ]), results_button_clicks or 0
                ...
                if isinstance(dt_value, datetime):
                    ...
                    if 'T' in dt_value:
                        ...
                        try:
                            ...
                            return dt_value.replace('T', ' ', 1).split('.')[0]  # Remove microseconds if present
                    else:
            ...
            if 'startTime' in events_for_simulation.columns:
                events_for_simulation['start_time'] = events_for_simulation['startTime'].apply(format_datetime_string)
            elif 'start_time' not in events_for_simulation.columns:
                return None, html.Div([
                    html.P("Missing 'startTime' or 'start_time' column in emission events.", 
                          style={'color': '#e74c3c', 'fontWeight': 'bold'})
                ]), results_button_clicks or 0
            
            if 'endTime' in events_for_simulation.columns:
                events_for_simulation['end_time'] = events_for_simulation['endTime'].apply(format_datetime_string)
            elif 'end_time' not in events_for_simulation.columns:
                return None, html.Div([
                    html.P("Missing 'endTime' or 'end_time' column in emission events.", 
                          style={'color': '#e74c3c', 'fontWeight': 'bold'})
                ]), results_button_clicks or 0
            ...
            if consider_wind_bool:
                if wind_data_store and 'data' in wind_data_store:
                    wind_speed = wind_data_store['data']
                else:
                    return None, html.Div([
                        html.P("⚠ Wind data is required when 'Consider Wind' is set to 'Yes'. Please upload wind data or use default.", 
                              style={'color': '#e67e22', 'fontWeight': 'bold'})
                    ]), results_button_clicks or 0
            ...
            if leak_data_store and 'data' in leak_data_store:
                ...
                if not isinstance(leak_data, list):
                    leak_data = leak_data.tolist() if hasattr(leak_data, 'tolist') else list(leak_data)
            ...
            # Returns total emissions (measured + unmeasured) per site
            total_emissions, total_emissions_lower, total_emissions_upper = simulate_below_mdl(
            ...
            simulation_result = {
                'below_mdl_emissions': total_emissions,  # Now represents total emissions (measured + unmeasured)
            ...
            rates = events_df['rate'].dropna().tolist() if 'rate' in events_df.columns else []
            durations = events_df['duration'].dropna().tolist() if 'duration' in events_df.columns else []
            
            if len(rates) == 0 or len(durations) == 0:
                return None, html.Div([
                    html.P("No rate or duration data found in emission events. Please check your data.", 
                          style={'color': '#e74c3c', 'fontWeight': 'bold'})
                ]), results_button_clicks or 0
            ...
            duration_dist = {"UNKNOWN": durations}
        ...
        processing_resolution,
            ...
            if extrapolation_results is None:
                return None, html.Div([
                    html.P(f"Extrapolation not implemented for processing resolution: {processing_resolution}", 
                          style={'color': '#e74c3c', 'fontWeight': 'bold'})
                ]), results_button_clicks or 0
            ...
            return None, html.Div([
                html.P(f"Unknown estimation approach: {estimation_approach}", 
                      style={'color': '#e74c3c', 'fontWeight': 'bold'})
            ]), results_button_clicks or 0
    ...
    if isinstance(simulation_result_serializable.get('start_time'), datetime):
        simulation_result_serializable['start_time'] = simulation_result_serializable['start_time'].isoformat()
    if isinstance(simulation_result_serializable.get('end_time'), datetime):
        simulation_result_serializable['end_time'] = simulation_result_serializable['end_time'].isoformat()
    
    # Update status message to show completion (replace the running message)
    status_message = html.Div([
        html.Div([
            html.H4("✓ Simulation Completed Successfully!", style={
                'color': '#27ae60',
                'marginBottom': '10px',
                'fontSize': '18px',
                'fontWeight': 'bold'
            }),
            html.P("Redirecting to Results page...", style={
                'color': '#2c3e50',
                'fontSize': '14px'
            })
        ], style={
    ...
    return simulation_result_serializable, status_message, (results_button_clicks or 0) + 1
...
def _format_simulation_results_display(simulation_result):
    ...
    if simulation_result is None:
        return html.Div([
            html.P("No simulation results available. Please run a simulation first.", 
                  style={'color': '#7f8c8d', 'fontStyle': 'italic'})
        ])
    ...
    # Display results
    results_content = [
        html.H4("Simulation Results:", style={'marginBottom': '10px', 'color': '#34495e'}),
        html.Div([
            html.P(f"Emission Estimation Approach: {approach_name}", 
                  style={'fontSize': '16px', 'fontWeight': 'bold', 'marginBottom': '10px', 'color': '#2c3e50'}),
            html.P(f"Processing Resolution: {simulation_result.get('processing_resolution', 'N/A')}", 
                  style={'fontSize': '16px', 'marginBottom': '10px'}),
            html.P(f"Measurement Technology: {simulation_result.get('measurement_technology', 'N/A')}" if simulation_result.get('measurement_technology') else f"Minimum Detection Limit: {simulation_result.get('minimum_detection_limit', 'N/A')} kg/hr", 
                  style={'fontSize': '16px', 'marginBottom': '10px'}),
        ])
    ]
    ...
    if start_time:
        ...
        results_content.append(
            html.P(f"Start Time: {start_time_display}", 
                  style={'fontSize': '16px', 'marginBottom': '10px'})
        )
    if end_time:
        ...
        results_content.append(
            html.P(f"End Time: {end_time_display}", 
                  style={'fontSize': '16px', 'marginBottom': '10px'})
        )
    
    # Add iterations info
    results_content.append(
        html.P(f"Number of Iterations: {simulation_result.get('monte_carlo_iterations', 'N/A')}", 
              style={'fontSize': '16px', 'marginBottom': '10px'})
    )
    ...
    if estimation_approach == 'below_MDL':
        ...
        if simulation_result.get('consider_wind'):
            wind_display = 'Yes' if simulation_result.get('consider_wind') == 'yes' else 'No'
            results_content.append(
                html.P(f"Consider Wind: {wind_display}", 
                      style={'fontSize': '16px', 'marginBottom': '10px'})
            )
        
        # Display total emissions results (measured + unmeasured)
        total_emissions = simulation_result.get('below_mdl_emissions')  # Legacy key name, but now contains total emissions
        ...
        if total_emissions is not None:
            results_content.append(
                html.Div([
                    html.H5("Total Emissions (Measured + Unmeasured):", 
                          style={'fontSize': '18px', 'fontWeight': 'bold', 'marginTop': '20px', 'marginBottom': '10px', 'color': '#2c3e50'}),
                    html.P(f"Total Emissions: {total_emissions:.2f} kg", 
                          style={'fontSize': '16px', 'marginBottom': '5px'}),
                    html.P(f"95% Confidence Interval: [{total_emissions_lower:.2f}, {total_emissions_upper:.2f}] kg", 
                          style={'fontSize': '16px', 'marginBottom': '10px', 'color': '#7f8c8d'}),
                    html.P("Note: Includes emissions from all measured events plus simulated unmeasured emissions below detection limit.", 
                          style={'fontSize': '14px', 'marginTop': '10px', 'color': '#95a5a6', 'fontStyle': 'italic'}),
                ])
            )
        ...
        if simulation_result.get('estimated_duration'):
            results_content.append(
                html.P(f"Estimated Duration: {simulation_result.get('estimated_duration', 'N/A')} hours", 
                      style={'fontSize': '16px', 'marginBottom': '10px'})
            )
                ...
                if len(emissions_list) > 0:
                    ...
                    results_content.append(
                        html.Div([
                            ...
                            html.Div([
                                html.H5(f"Statistics ({key}):", style={'marginTop': '20px', 'marginBottom': '10px'}),
    ...
    return html.Div([
        html.Div(results_content, style={
            'padding': '15px',
...
def display_simulation_results(section_content, simulation_results):
...
def handle_bottomup_inventory(n_clicks, emissions_value, lower_uncertainty, upper_uncertainty, 
                              simulation_results, existing_bottomup):
    ...
    if emissions_value is None or emissions_value <= 0:
        return existing_bottomup, html.Div([
            html.P("Please enter a valid emissions value (kg).", 
                  style={'color': '#e74c3c', 'fontStyle': 'italic', 'marginTop': '10px'})
        ])
    ...
    if simulation_results is None:
        return bottomup_data, html.Div([
            html.P("Bottom-up inventory saved. Run a simulation to see comparison.", 
                  style={'color': '#27ae60', 'fontStyle': 'italic', 'marginTop': '10px'})
        ])
    ...
    if simulation_value is None:
        return bottomup_data, html.Div([
            html.P("Bottom-up inventory saved. Simulation results do not contain valid emission values.", 
                  style={'color': '#e67e22', 'fontStyle': 'italic', 'marginTop': '10px'})
        ])
    ...
    return bottomup_data, html.Div([
        html.P("Bottom-up inventory added successfully!", 
              style={'color': '#27ae60', 'fontWeight': 'bold', 'marginTop': '10px', 'marginBottom': '15px'}),
        dcc.Graph(figure=fig)
    ])
...
@app.callback(
    Output('uncertainty-source-dropdown', 'options'),
    [Input('stored-events', 'data')]
)
...
@app.callback(
    [Output('continuous-emissions-container', 'style'),
     Output('intermittent-uncertainty-container', 'style'),
     Output('source-selection-container', 'style'),
     Output('stored-emission-characteristics', 'data')],
    [Input('emission-characteristics-dropdown', 'value')]
)
def toggle_emission_characteristics_ui(emission_char):
    """Show/hide UI containers based on emission characteristics selection"""
    if emission_char == 'continuous':
        continuous_style = {'marginTop': '20px', 'display': 'block'}
        intermittent_style = {'marginTop': '20px', 'display': 'none'}
        source_style = {'marginTop': '20px', 'display': 'none'}  # Hide source selection for continuous
    elif emission_char == 'intermittent':
        continuous_style = {'marginTop': '20px', 'display': 'none'}
        intermittent_style = {'marginTop': '20px', 'display': 'block'}
        source_style = {'marginTop': '20px', 'display': 'block'}  # Show source selection for intermittent
    else:
        continuous_style = {'marginTop': '20px', 'display': 'none'}
        intermittent_style = {'marginTop': '20px', 'display': 'none'}
        source_style = {'marginTop': '20px', 'display': 'none'}
    
    return continuous_style, intermittent_style, source_style, {'value': emission_char} if emission_char else None
...
def confirm_producing_hours(n_clicks, producing_hours):
    ...
    if producing_hours is None:
        return html.Div([
            html.P("Please enter producing hours.", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold'})
        ]), None
    
    if producing_hours < 0 or producing_hours > 8760:
        return html.Div([
            html.P("Producing hours must be between 0 and 8760.", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold'})
        ]), None
    
    # Store the value
    producing_hours_data = {
        'hours': producing_hours
    }
    
    status = html.Div([
        html.P(f"✓ Producing hours confirmed: {producing_hours} hours", 
              style={'color': '#27ae60', 'fontWeight': 'bold'})
    ])
...
def plot_uncertainty_events(selected_source, consider_uncertainty, uncertainty_method, 
                            manual_rate, simulation_results, emission_char_data, events_data, loaded_data):
    """Plot emission events over time for selected source"""
    if events_data is None or len(events_data) == 0:
        return html.Div([
            html.P("No emission events data available. Please convert data to emission events first.", 
                  style={'color': '#7f8c8d', 'fontStyle': 'italic'})
        ]), html.Div()
    ...
    if selected_source is None and not is_continuous:
        return html.Div([
            html.P("Please select a source to view events.", 
                  style={'color': '#7f8c8d', 'fontStyle': 'italic'})
        ]), html.Div()
    ...
    if consider_uncertainty == 'yes':
        if simulation_results is None or len(simulation_results) == 0:
            return html.Div([
                html.P("Please run duration uncertainty estimation first, or select 'No' for duration uncertainty.", 
                      style={'color': '#7f8c8d', 'fontStyle': 'italic'})
            ]), html.Div()
    ...
    if source_col not in events_df.columns:
        return html.Div([
            html.P("Source column not found in emission events data.", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold'})
        ]), html.Div()
    ...
    if len(filtered_events) == 0:
        return html.Div([
            html.P(f"No events found for source: {selected_source}", 
                  style={'color': '#7f8c8d', 'fontStyle': 'italic'})
        ]), html.Div()
    ...
    if consider_uncertainty == 'yes' and simulation_results is not None:
        ...
        if 'id' in sim_df.columns:
            filtered_events = filtered_events.merge(
                sim_df[['id', 'simulated_duration', 'simulated_duration_lower', 'simulated_duration_upper']],
                on='id',
                how='left'
            )
        else:
            ...
            filtered_events = pd.concat([
                ...
                sim_df[['simulated_duration', 'simulated_duration_lower', 'simulated_duration_upper']].reset_index(drop=True)
            ], axis=1)
    ...
    for idx, row in filtered_events.iterrows():
        ...
        if pd.notna(start_time) and pd.notna(rate):
            try:
                ...
                if consider_uncertainty == 'yes' and pd.notna(row.get('simulated_duration')):
                    ...
                    duration_lower = float(row.get('simulated_duration_lower', simulated_duration))
                    duration_upper = float(row.get('simulated_duration_upper', simulated_duration))
                else:
    ...
    if len(plot_data) == 0:
        return html.Div([
            html.P(f"No valid time/rate data found for source: {selected_source}", 
                  style={'color': '#7f8c8d', 'fontStyle': 'italic'})
        ]), html.Div()
    ...
    for idx, row in plot_df.iterrows():
        ...
        if consider_uncertainty == 'yes' and row['duration_lower'] != row['duration_upper']:
            ...
            cap_height = (row['rate_upper'] - row['rate_lower']) * 0.1  # 10% of rate range
            fig.add_trace(go.Scatter(
                x=[x_lower, x_lower],
    ...
    hover_texts = []
    for idx, row in plot_df.iterrows():
        hover_text = (f"Event ID: {row['event_id']}<br>"
    ...
    fig.add_trace(go.Scatter(
        x=plot_df['start_datetime'] + (plot_df['end_datetime'] - plot_df['start_datetime']) / 2,
        y=plot_df['rate'],
        mode='markers',
    ...
    if consider_uncertainty == 'yes' and simulation_results is not None and len(simulation_results) > 0:
        ...
        display_cols = ['id', 'rate', 'duration', 'simulated_duration', 
                       'simulated_duration_lower', 'simulated_duration_upper', 
                       'quantity', 'sourceLocation', 'startTime', 'endTime']
        available_cols = [col for col in display_cols if col in filtered_with_sim.columns]
        table_df = filtered_with_sim[available_cols].copy()
        
        # Format numeric columns
        if 'rate' in table_df.columns:
            table_df['rate'] = table_df['rate'].apply(lambda x: f"{x:.4f}" if pd.notna(x) else "N/A")
        if 'duration' in table_df.columns:
            table_df['duration'] = table_df['duration'].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
        if 'simulated_duration' in table_df.columns:
            table_df['simulated_duration'] = table_df['simulated_duration'].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
        if 'simulated_duration_lower' in table_df.columns:
            table_df['simulated_duration_lower'] = table_df['simulated_duration_lower'].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
        if 'simulated_duration_upper' in table_df.columns:
            table_df['simulated_duration_upper'] = table_df['simulated_duration_upper'].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
        if 'quantity' in table_df.columns:
            table_df['quantity'] = table_df['quantity'].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
        
        # Create table HTML
        events_table = html.Div([
            html.H4("Output Events with Duration Uncertainty", style={'marginTop': '20px', 'marginBottom': '10px'}),
            html.Div([
                html.Table([
                    html.Thead([
                        html.Tr([html.Th(col.replace('_', ' ').title(), style={
                            'padding': '8px',
                            'textAlign': 'left',
                    ...
                    html.Tbody([
                        html.Tr([
                            html.Td(str(table_df.iloc[i][col]), style={
                                'padding': '8px',
    ...
    try:
        ...
        if method == 'default':
            ...
            if manual_rate is None or manual_rate == '':
                return html.Div([
                    html.P("Please provide a leak production rate value.", 
                          style={'color': '#e74c3c', 'fontWeight': 'bold'})
                ]), None, events_data
            ...
            return html.Div([
                html.P("Invalid method selected.", 
                      style={'color': '#e74c3c', 'fontWeight': 'bold'})
            ]), None, events_data
        ...
        transformed_events['start_time'] = transformed_events['startTime'].apply(format_datetime)
        transformed_events['end_time'] = transformed_events['endTime'].apply(format_datetime)
        ...
        if len(valid_events) == 0:
            return html.Div([
                html.P("No valid events with start and end times found.", 
                      style={'color': '#e74c3c', 'fontWeight': 'bold'})
            ]), None, events_data
        ...
        if 'simulated_duration' in result_events.columns:
            num_simulated = len(result_events[result_events['simulated_duration'].notna()])
            ...
            result_cols = ['id', 'simulated_duration', 'simulated_duration_lower', 'simulated_duration_upper']
            available_cols = [col for col in result_cols if col in result_events.columns]
            ...
            if 'id' in events_df.columns and 'id' in sim_results_df.columns:
                ...
                updated_events_df = events_df.merge(
                    sim_results_df[['id', 'simulated_duration', 'simulated_duration_lower', 'simulated_duration_upper']],
                    on='id',
                    how='left'
                )
            else:
                ...
                updated_events_df = pd.concat([
                    events_df_reset,
                    sim_results_reset[['simulated_duration', 'simulated_duration_lower', 'simulated_duration_upper']]
                ], axis=1)
            ...
            status_div = html.Div([
                html.P("✓ Duration uncertainty estimation completed successfully!", 
                      style={'color': '#27ae60', 'fontWeight': 'bold', 'marginTop': '10px'}),
                html.P(f"Simulated durations for {num_simulated} events", 
                      style={'color': '#27ae60', 'fontSize': '14px'}),
                html.P(f"Leak Production Rate used: {leak_production_rate:.6f} kg/hr", 
                      style={'color': '#7f8c8d', 'fontSize': '14px', 'marginTop': '5px'}),
                html.P(f"Monte Carlo iterations: {MC_iterations} (parallel processing enabled)", 
                      style={'color': '#7f8c8d', 'fontSize': '12px', 'fontStyle': 'italic', 'marginTop': '5px'}),
            ])
            
            return status_div, simulation_results, updated_events_data
        else:
            return html.Div([
                html.P("Simulation completed but no simulated durations were generated.", 
                      style={'color': '#e67e22', 'fontWeight': 'bold'})
            ]), None, events_data
            
    except Exception as e:
        return html.Div([
            html.P(f"✗ Error during duration uncertainty estimation: {str(e)}", 
                  style={'color': '#e74c3c', 'fontWeight': 'bold', 'marginTop': '10px'})
        ]), None, events_data
...
def download_emission_events(csv_clicks, json_clicks, events_data):
    ...
    if button_id == 'download-csv-button':
        ...
        # Download as JSON
        return dict(content=df.to_json(orient='records', indent=2), filename="emission_events.json")
